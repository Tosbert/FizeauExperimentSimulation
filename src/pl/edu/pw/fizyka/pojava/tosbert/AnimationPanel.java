...
public class AnimationPanel extends JPanel {
	...
	/**
	 * Hubert Nowakowski
	 * Panel obsługujący tworzenie koła zębatego, promienia światła
	 * i innych elementów układu oraz odpowiednie ich wyświetlanie.
	 */
	...
	AnimationPanel(int vel ,int n ,int d, double width, double height){
		...
		int fullMirrorY = (int)( this.wheel.getY()+this.wheel.getHeight()/4);
		this.fullMirror = new Rectangle( fullMirrorX , fullMirrorY ,15 , (int)(height*0.2) );
		...
		int xPoly[] = {(int)( this.lightReciever.getX()+width*0.03 ) ,
				(int)( this.lightReciever.getX()-width*0.03 ),
				(int)( this.lightReciever.getX()-width*0.02 ),
				(int)( this.lightReciever.getX()+width*0.04 )};
		int yPoly[] = {partialMirrorTopHeight ,
				partialMirrorTopHeight + (int)(height*0.2),
				partialMirrorTopHeight + (int)(height*0.2),
				partialMirrorTopHeight};
	...
	public void paintComponent(Graphics g ){
		...
		for(WheelTooth r : this.wheelTeeth)
		{
			g2d.draw(r);
			g2d.fill(r);
		}
		g2d.setColor(Color.WHITE);
		g2d.fill(new Rectangle((int)this.wheel.getX()-5,
				(int)(this.wheel.getY()+this.wheel.getHeight()),
				(int)(this.wheel.getWidth())+10,
				getHeight() - (int)(this.wheel.getY()+this.wheel.getHeight()) ));
		
		g2d.fill(new Rectangle((int)this.wheel.getX()-5,
				(int)this.wheel.getY()-100,
				(int)(this.wheel.getWidth())+10,
				100));
	...
	public void makeLightBeam(){ 
		...
		this.lightBeam.add( new Rectangle( (int)this.wheel.getX(), 
				(int)(this.wheel.getY()+this.wheel.getHeight()/2)-5,
				(int) (this.fullMirror.getX()-(int)this.wheel.getX()),
				3) 
				);
		this.lightBeam.add( new Rectangle( (int)(this.lightReciever.getX()+(this.lightReciever.getWidth()/2)), 
				(int)(this.wheel.getY()+this.wheel.getHeight()/2) -5 ,
				(int) (this.fullMirror.getX()-(int)(this.lightReciever.getX()+(this.lightReciever.getWidth()/2))),
				3) 
				);
	}
